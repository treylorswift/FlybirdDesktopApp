...
class TwitterFollowerCacheBase {
    ...
    GetStatus() {
        ...
        if (!p)
            return { status: ServerApi_1.FollowerCacheStatusEnum.None, completionPercent: 0, totalStoredFollowers: 0 };
        let totalStoredFollowers = this.GetNumStoredFollowers();
        //if we're actively caching now, say so.. include completion percent
        if (this.buildInProgress) {
            return { status: ServerApi_1.FollowerCacheStatusEnum.InProgress, completionPercent: p.completionPercent, totalStoredFollowers: totalStoredFollowers };
        }
        //we're not in progress..
        //there's a record but its not finished. Incomplete
        if (!p.finishTime)
            return { status: ServerApi_1.FollowerCacheStatusEnum.Incomplete, completionPercent: p.completionPercent, totalStoredFollowers: totalStoredFollowers };
        //there is a record, it shows complete. 
        return { status: ServerApi_1.FollowerCacheStatusEnum.Complete, completionPercent: p.completionPercent, totalStoredFollowers: totalStoredFollowers };
    }
    //@ts-ignore
    async Build() {
        try {
...
class TwitterFollowerCacheSQL extends TwitterFollowerCacheBase {
    ...
    async Query(q) {
        ...
        try {
            let queryString = `SELECT TwitterFollowers.id_str,
